...
protocol EditUsersCellDelegate: class {
    func showImageAttachment()
}

class EditUsersCell: UITableViewCell {
    ...
    weak var delegate: EditUsersCellDelegate?
    var detailObj: UsersDetailModel? {
        didSet {
            cellConfig()
        }
    }
    ...
    func cellConfig() {
        ...
        nameText.text = obj.user_detail_name
        addressText.text = obj.user_detail_address
        emailText.text = obj.user_detail_email
        genderButton.setTitle(obj.user_detail_gender, for: .normal)
//        phoneCodeAreaTxt.text = "\(obj.user_detail_phone.prefix(2))"
        phoneText.text = String(obj.user_detail_phone.dropFirst(2))
    }
...
extension EditUsersCell: UITextFieldDelegate {
...
extension EditUsersCell: FPNTextFieldDelegate {
