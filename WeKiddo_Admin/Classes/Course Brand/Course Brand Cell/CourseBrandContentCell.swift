...
protocol CourseBrandContentCellDelegate: class {
    func goToCourseDetail()
    func goToMore()
}

class CourseBrandContentCell: UITableViewCell, UICollectionViewDelegate, UICollectionViewDataSource {

    @IBOutlet weak var moreButton: UIButton!
    @IBOutlet weak var sectionLabel: UILabel!
    @IBOutlet weak var collectionView: UICollectionView!
    var detailObj: [CourseBrandSubjectModel]! {
        didSet {
            self.collectionView.reloadData()
        }
    }
    var indexCurrent = 0
    weak var delegate: CourseBrandContentCellDelegate?
...
//            self.delegate?.goToMore()
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.contentTitleLabel.text = detailObj[indexPath.row].subject_name
        cell.contentImage.sd_setImage(
            with: URL(string: detailObj[indexPath.row].franchise_image),
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print(detailObj[indexPath.row].franchise_id)
        fetchCourseDetail(courseID: detailObj[indexPath.row].header_id)
    }
}
